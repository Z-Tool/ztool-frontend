    ...
    <el-row :gutter="20">
      <el-col :span="6" :offset="9">
        <el-form :model="loginForm" :rules="rules" ref="loginForm" class="loginForm">
          ...
          <div class="create">
            <router-link to="/">
              <el-button type="warning" style="width: 100%;">Cancel</el-button>
            </router-link>
            <p>Not have an account?</p>
            <router-link to="/user/new">
              <el-button style="width: 100%;">Create an account</el-button>
            </router-link>
          </div>
        </el-form>
      </el-col>
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            const authStr = window.btoa(this.loginForm.username + ':' + this.loginForm.password);
            const headers = {
